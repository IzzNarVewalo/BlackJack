...
public class HandOnline {
    ...
    public static int kartenZaehlen(boolean Spieler){
        ...
        if (Spieler) {
            for (int i = 0; i < GameOnline.getSpieler().getKarten().size(); i++) {
                gesamtBlackJackWert += GameOnline.getSpieler().getKarten().get(i).getBlackJackWert();
            ...
            for (int i = 0; i < GameOnline.getDealer().getKarten().size(); i++){
                gesamtBlackJackWert += GameOnline.getDealer().getKarten().get(i).getBlackJackWert();
    ...
    private static int asseZaehlen(boolean spieler){
        ...
        if (spieler) {
            for (int i = 0; i < GameOnline.getSpieler().getKarten().size(); i++) {
                if (GameOnline.getSpieler().getKarten().get(i).getZeichenWert() == 14)
                    counter++;
            }
        } else {
            for ( int i = 0; i < GameOnline.getDealer().getKarten().size(); i++){
                if (GameOnline.getDealer().getKarten().get(i).getZeichenWert() == 14)
                    counter++;
            }
        }
        return counter;
